...
class BookspiderSpider(scrapy.Spider):
    ...
    def parse_book_page(self, response):
        ...
        if not book:
            self.logger.error(f"No book found at {response.url}")
            return
        ...
        book_item['stars'] = book.css("p.star-rating").attrib.get('class', '')
        book_item['category'] = response.xpath("//ul[@class='breadcrumb']/li[@class='active']/preceding-sibling::li[1]/a/text()").get()
        book_item['description'] = response.xpath("//div[@id='product_description']/following-sibling::p/text()").get()
        book_item['price'] = book.css('p.price_color ::text').get()

        # Ensure no field is None
        for field in book_item.fields:
            if book_item.get(field) is None:
